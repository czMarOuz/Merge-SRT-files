...
def ziskat_delku_posledniho_souboru(soubor):
    ...
    with open(soubor, 'r', encoding='utf-8') as f:
        ...
        # Hledá poslední řádek s časovým označením
        for radek in reversed(radky):
            if "-->" in radek:
                posledni_radka = radek
                break
        ...
        if posledni_radka:
            ...
            # Pokud se nepodaří najít časové označení, vrátíme 0
            return 0
...
def posunout_casova_oznaceni(casova_oznaceni, posun_casu):
    ...
    for cas in casova_oznaceni:
        ...
        # cas_bez_carky = re.sub(r',0+$', '', cas) #původní
        cas_bez_carky = re.sub(r',\d+$', '', cas)
        # print(f"o kolik se má čas posunout v milisekundách: {posun_casu}")
        
        # hodiny, minuty, vteriny = map(int, cas_bez_carky.split(":")) #původní
